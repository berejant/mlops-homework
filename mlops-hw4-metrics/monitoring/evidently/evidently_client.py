...
class EvidentlyClient:
    ...
    def create_and_upload_drift_report(self, reference_dataset_id: str, current_df: pd.DataFrame) -> str:
        ...
        try:
            ...
            logger.info(f"Creating/getting project with ID: {Config.EVIDENTLY_PROJECT_ID}")
            self.create_or_get_project()
            
            # Завантажуємо reference набір даних з Cloud
            logger.info(f"Downloading reference dataset: {reference_dataset_id}")
            ...
            logger.info(f"Uploading report to project: {self.project.id}")
            ...
            logger.info(f"Report created successfully: {report_url}")
            
            return report_url
            
        except Exception as e:
            logger.error(f"Error creating drift report: {str(e)}")
            logger.error(f"Project ID: {Config.EVIDENTLY_PROJECT_ID}")
            logger.error(f"Reference Dataset ID: {reference_dataset_id}")
            logger.error(f"API Key present: {bool(Config.EVIDENTLY_API_KEY)}")
            raise 